...
export default class Viewweb extends LightningElement {
    ...
    connectedCallback() {
        ...
        getLWCList().then(result => {
            ...
            if (data.records) {
                ...
                this.containerForWebBundleResult = [];
                this.showResult = '';
            } else {
            ...
            const event = new ShowToastEvent({
                title: 'Remote Site Settings Reqired.',
                message: 'Add your org\'s base url to remote site settings,Check setup help',
                variant: 'error',
                mode: 'pester'
            });
    ...
    handleSelectTreeSelect(event) {
        let that = this;
        let selectedId = event.currentTarget.value;//event.detail.name;
        ...
        if (recordData.length > 0) {
            if (recordData[0].attributes.type === 'LightningComponentBundle') {
                ...
                if (bundleData.length > 0) {
                    ...
                    let resultRefresh = that.resultList;
                    resultRefresh.forEach(function (findingnode) {
                        if (findingnode.name == selectedId) {
                            findingnode.expanded = !findingnode.expanded;
                        }
                    });
                    that.resultList = resultRefresh;
                ...
                queryWebComponent({
                    ...
                    //all component data fetched from server
                    let data = JSON.parse(result);
                    
                    that.containerForWebBundleResult = that.containerForWebBundleResult.concat(data.records);

                    //removing dublicate
                    let things = new Object();
                    things.thing = that.containerForWebBundleResult;
                    let myData = things.thing;
                    things.thing = Array.from(new Set(myData.map(JSON.stringify))).map(JSON.parse);
                    that.containerForWebBundleResult = things.thing;
                    ...
                    resultRefresh.forEach(function (findingnode) {
                        if (findingnode.name == selectedId) {
                            let tempList = [];
                            data.records.forEach(function (continerRecord) {
                                let rempRecrd = {
